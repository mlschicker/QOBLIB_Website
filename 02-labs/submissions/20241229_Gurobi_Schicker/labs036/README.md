# Submission for labs036

This directory contains the submission for the problem **labs036**.

| Field | Value 1 |
| --- | --- |
| Problem | labs036 |
| Submitter | Maximilian Schicker |
| Affiliation | Zuse Institute Berlin |
| Date | 2024-12-28 18:00:07 |
| ====== |  |
| Reference | See Models Directory (integer) using Gurobi 11.0.0 |
| Best Objective Value | 82.0 |
| Optimality Bound | 82.0 |
| ====== |  |
| Modeling Approach | Integer Program |
| # Decision Variables | 700 |
| # Binary Variables | 665 |
| # Integer Variables | 35 |
| # Continuous Variables | 0 |
| # Non-Zero Coefficients | 0 |
| Coefficients Type | Integer |
| Coefficients Range | 0.0 - 0.0, 4.0 - 4.0, 1.0 - 4.0 |
| ====== |  |
| Workflow | Generate LP files using ZIMPL, solve using Gurobi |
| Algorithm Type | Deterministic |
| Paradigm | Classical |
| # Runs | 1 |
| # Feasible Runs | 1 |
| # Successful Runs | 1 |
| Success Threshold | 0.0001 |
| ====== |  |
| Hardware Specifications | linux64, gurobi_cl: AMD EPYC 7542 32-Core Processor, instruction set [SSE2\|AVX\|AVX2] |
| ====== |  |
| Total Runtime | 5610.1 |
| Time to Solution | 2111 |
| CPU Runtime | 5214.71 |
| GPU Runtime | N/A |
| QPU Runtime | N/A |
| Other HW Runtime | N/A |
| ====== |  |
| Remarks | CPU Runtime is Gurobi Work Measure, Success Threshold is the MIP Gap tolerance, variable counts are after presolve, Coefficient Range is (Min - Max) for (Linear, Quadratic, Quadratic Linear) coefficients |
